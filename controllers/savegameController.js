...
const createSavegame = async (req, res) => {                    
    ...
    try {
        ...
        // only allow 3 saveslots per user

        if (checkSaveAmount.length < 3) {
            const savegame = new Savegame({ user: user_id, name: data.name, experience: data.experience, level: data.level, money: data.money, unlocks: data.unlocks, day: data.day, stats: data.stats, buffs: data.buffs, items: data.items, date_created: data.date_created })
            await savegame.save()   
...
const updateSavegame = async (req, res) => {                    
    ...
    try {
        ...
        const update = await Savegame.updateOne({ _id: savegame_id }, { name: data.name, money: data.money, experience: data.experience, level: data.level, unlocks: data.unlocks, day: data.day, stats: data.stats, buffs: data.buffs, items: data.items, date_active: date}).exec()
